...
`D:\ai\yexuan_memery\docs\protocols\sensor-event-protocol.md` 已提出 `privacy.allow_memory`
草案，但未落地。
...
是否已有记录：  
`D:\ai\yexuan_memery\docs\protocols\sensor-event-protocol.md` 已建议 `ephemeral` 默认不进记忆；
当前代码未实现。
...
- `D:\ai\qq-st-bot\docs\memory.md` 情景记忆上限段说明当前裁剪保护。
