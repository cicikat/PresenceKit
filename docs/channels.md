...
- 梦境邀请：Path B 推送 `dream_invite` action；Emerald-client 收到并 ack 后打开 Dream 窗口。
- 玩耍邀请：Path B 推送 `toy_invite` action；Emerald-client 在「玩耍模式」开关开启时打开 ToyWindow，关闭时忽略并正常回 ack。
