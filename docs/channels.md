...
桌面端当前正式协议为 **v0.1（legacy 冻结版）**。本仓只维护实现说明，协议消息全集、ack/nack 语义、9 类 desktop action allowlist 与 HTTP/WS 对账契约统一见三仓总账 [three-repo-interface-catalog.md](three-repo-interface-catalog.md) 和桌面仓的 `docs/protocol-v0.md`。固定会话 scope 是加法能力 `session_scope=v1`：新客户端先绑定 session，再以 header 调用既有路由；早期 stream/canonical/segments 可带 `request_id` 与冻结角色。旧客户端省略 header 时仍保持 v0.1 live-active 行为。详见 [session-scope-contract.md](session-scope-contract.md)。
