...
`X-Presence-Session` 的 desktop/mobile chat、upload、wake、media、chat-log/calendar 与
reasoning 全部复用冻结 owner/char；无 header 仍是 legacy live active。
`request_id` 有 30 分钟有界 receipt；同 payload 重放、冲突、in-flight 和 unknown-result
...
| `/memory-events/search`、`/memory-events/{event_id}`、`/memory-events/{event_id}/window`、`/memory-events/{event_id}/related`、`/memory-events/query-trace`、`/memory-events/lineage/*`、`/observability/memory-event-edge-proposals`、`/observability/memory-event-shadow-recall` | Memory Event 证据账本的 scoped 只读检索、受控 topics、确定性关联边、派生记忆血缘、脱敏查询审计、候选边调度和 shadow recall 观测 | `admin-only`；前者需 `memory.read`，请求必须显式 `uid + char_id + realm=reality`，`related` 对同一邻居保留兼容首关系并返回完整 `relations[]`；两类观测需 `state.read`，候选边只含 run/budget、发现和超时计数，shadow 只含事件/turn 覆盖、mapped/unmapped、超时指标和待批准 `retirement_draft`（`used_as_gate=false`）。默认过滤 `web`/`dream_echo`/`coplay`，管理员只能以显式 `source` 查看隔离证据，响应保留 source 标签；lineage 仅消费落盘的事件 ID，旧数据/已删除事件为 `legacy_unknown`，dry-run 不写回；管理面消费，桌面/手机不消费，不进入 prompt 或 tool loop |
| `DELETE /memory-events/{event_id}`、`GET /observability/memory-event-migration` | Memory Event 10 的可逆遗忘与迁移进度 | `admin-only`；前者需 `admin`，只墓碑化正文/媒体引用并保留关系边，后者需 `state.read`，返回无正文的计数、当前/旧 Markdown 来源覆盖、`inventory_only` 资产摘要和待批准 `retirement_draft`；桌面/手机不消费，管理面事件证据页消费，物理删除为 `disabled_pending_owner_policy`。阈值未批准前不得删旧召回 |
...
短时内容指纹兜底，不能把指纹当正式协议键。body 不接受权威 `char_id`；固定会话由
`X-Presence-Session` 提供，见 [session-scope-contract.md](session-scope-contract.md)。
...
| `workspace_list/read/create/update/delete/undo` | Backend Reality tool loop; local deployment and explicit configured roots only | `current`, backend; Dream and `remote_server` fail closed, writes return Task receipts whose `causation_ref.kind` is `tool_request` (request fingerprint), not `reality_turn` |
...
现有 GET /admin/autonomy/tools 用唯一 AutonomyToolDecision（allowed / decision_source / denial_reason=night_no_active_device）解释拒绝，
