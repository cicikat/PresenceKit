...
这个脚本只适用于 Git clone，不是 Release zip 的更新器。它会先确认服务进程已停止；发现
未提交改动时展示 `git status --porcelain` 并要求明确输入 `Y` 才会继续。脚本不会执行 reset、
stash 或删除 `data/`、`config.yaml`、secrets，但同一文件的本地改动仍可能让 `git pull` 发生冲突。

拉取成功后脚本才同步锁定依赖；依赖同步失败不会回滚已拉取的源码，应先保留终端错误再处理网络或
环境问题。若当前目录是解压的 Release 包，按脚本提示下载新版包，并且只覆盖程序代码，保留现有
`data/`、配置和 secrets。
