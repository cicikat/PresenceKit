...
| 角色认知 | `data/character_growth/叶瑄_{uid}.md` | fixation pipeline 达阈值后固化更新 | 层6a |
...
- 更新 `fixation_state`，达阈值后入队 `consolidate_to_growth`
...
**上限**：最多 200 条，超过时按 strength 排序删掉最低的 20 条。注意：当前 `write_episode()` 的上限裁剪没有特殊保护 `is_core`，这是已知技术债，见 `docs/known-issues.md`。
...
**核心记忆标记**：`is_core=True` 会在格式化时标出【重要】，但当前 `retrieve()` 排序没有额外提前核心记忆。
...
| 读取方式 | 内容 | 触发条件 |
|---|---|---|
| prompt fingerprint | 从 `.felt.md` 或 `.md` 取前 150 字 | tagged 未命中时 |
| prompt full | `.felt.md` 或 `.md` 完整内容 | tagged：`topic.relation / topic.history / emotion.deep / meta.identity / emotion.down` |
| `load_fingerprint()` | `.fingerprint.txt` 或 `.md` 前 150 字 | 兼容接口，当前 prompt_builder 主路径未调用 |
| `load()` | `.md` 全文 | fetch_context 固定读取，后续由 prompt_builder 决定截断或全文注入 |
...
当前主路径 `fixation_pipeline.consolidate_to_growth()` 没有调用 `trait_tracker`，因此如果只走新固化 pipeline，`trait_state.json` 可能不会刷新。这是已知技术债，见 `docs/known-issues.md`。
...
包括 capture_turn、mid_term、user_profile、character_growth、episodic_memory 等。
