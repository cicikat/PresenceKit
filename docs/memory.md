...
| episodic 淘汰归档（时期摘要，已退役） | `data/runtime/memory/{char_id}/{uid}/memory_digest.md` | **不再写入**——Brief 80 §3 起，episodic 淘汰批次改走 storyline_inbox；存量文件原地只读保留，`GET /memory/digest/{user_id}` 仍可读 | 不注入 prompt |
...
**遗忘=降级而非删除（Brief 46 §1，Brief 80 §3 起改道）**：被裁的 20 条不是直接丢弃——
`write_episode` 会先把条目全文快照 + `char_id` 入队 slow_queue 任务
`storyline_evicted_input`，再从 episodic.json 删除。handler
（`core/memory/fixation_pipeline.py::handler_storyline_evicted_input`）**不再做即时 LLM
压缩**——只把 `{id, summary, ts, strength}` 追加进 `storyline_inbox.json`（滚动保留 200 条），
等 `storyline_weekly` 周频聚合统一消费+清空，省一路即时 LLM 调用。原 `digest_evicted_episodes`
job（即时压缩写 `memory_digest.md`）已随之删除；存量 `memory_digest.md` 文件原地只读保留，
`GET /memory/digest/{user_id}` 仍可读，但不会再有新内容写入。详见下方「storyline 叙事弧层」。
...
**维护要点**：`get_paths().yexuan_traits(char_id=...)` 读取的 traits 与
`get_paths().author_notes_pool(char_id=...)` 读取的 note，其 trait `id` / `trait_ids` 必须精确一致，
否则权重翻倍会静默失效。当前主路径为 `userdata/characters/authored/{char_id}/`；仅旧安装会回退
`content/characters/` 或更早的兼容种子。

---

## 四点六、storyline 叙事弧层（Brief 80）

**文件**：`core/memory/storyline.py`（存储+写API）、`core/scheduler/triggers/storyline_weekly.py`（聚合）
**存储**：`data/runtime/memory/{char_id}/{uid}/storyline.json`（弧线）+ `storyline_inbox.json`（待聚合暂存）+ `storyline_archive.md`（超限淘汰归档）

### 定位

`user_identity` 回答"他是个什么样的人"（跨情景反复出现、无特定时间跨度的稳定属性）；
storyline 回答"他在经历什么弧线"（有时间跨度的叙事：职业方向转变、一个项目的推进、
一段持续的情绪历程）。两层类型互斥，写入路由层各自的 prompt 显式排除对方的内容类型
（见 §三.3 00d 备忘的类型隔离设计）——`consolidate_to_identity` 的
system prompt 第 10 条规则、storyline 聚合 prompt 的开头都各自声明了这条边界。
...
```json
{
  ...
  "arcs": [
    {
      "arc_id": "arc_xxxxxxxx", "title": "≤20字", "status": "active|dormant|closed",
      "tags": ["topic.learning"],
      "nodes": [{"node_id": "n_xxxxxxxx", "ts": 0.0, "span": [0.0, 0.0], "summary": "≤80字", "source_ids": []}],
...
**append-only 硬约束（00d 裁决 1：增量式 + 旧节点只读只追加）**：公开写 API 只有
`open_arc()` / `append_node()` / `set_arc_status()`——不存在修改既有 node 的公开路径。
`append_node` 拒绝 ts 早于该 arc 最后一个 node（防伪造历史）、拒绝重复 node_id、
达 `MAX_NODES_PER_ARC`（40）后拒绝写入。总 arcs 超过 `MAX_TOTAL_ARCS`（24）时淘汰最旧的
closed arc 整条追加进 `storyline_archive.md` 再移除；`MAX_ACTIVE_ARCS`（8）是软上限，
由聚合 prompt 感知并自行收敛（关闭旧弧线），不做代码强制淘汰。

### 聚合：storyline_weekly（周频，挂 scheduler，不挂慢队列）

00d 备忘 §三.2 的判断：slow_queue 是逐轮驱动链（capture_turn → mid_term → episodic →
identity），周频节奏在这条链上没有触发器；正确落点是 scheduler trigger，照
`hidden_state_decay._check_hidden_state_consolidate` 的 7 天冷却模式。

每次运行一次 LLM 调用，输入三路：
1. 上次聚合后新增的 episodic 条目（含已被 identity 固化的——两层互不排斥）；
2. `storyline_inbox.json` 的 episodic 淘汰批次碎片（见下）；
...
原 `digest_evicted_episodes`（Brief 46 §1，episodic 淘汰批次即时 LLM 压缩写
`memory_digest.md`）已删除。episodic 淘汰批次现在只做一件事：`handler_storyline_evicted_input`
把 `{id, summary, ts, strength}` 追加进 `storyline_inbox.json`（滚动保留 200 条），
等 `storyline_weekly` 统一消费+清空，省一路即时 LLM 调用。存量 `memory_digest.md` 文件
原地只读保留，`GET /memory/digest/{user_id}` 仍可读，但不会再有新内容写入。

### 注入：层 `6h_storyline`（tagged，relevance 门控，非常态）

同 dream 印象 / watch / growth_self 的 tagged 惯例：只在本轮 `_tags` 与某条
active/dormant 弧线的 `tags` 有交集时才注入交集最多的那一条（title + 最近 ≤3 个 node
summary，≤300 字），backchannel 低信息轮跳过（`recall_gate.is_low_information()`）。
`_drop_priority=65`，介于 `mid_term`(40) 与 `episodic`(70) 之间。
...
**存哪**：`data/runtime/characters/{char_id}/inner/mood_state.json`（角色级唯一，不区分用户）
...
reflect_to_episodic  ←─── episodic_sweep（调度器，冷却 30min，aged > 11h）
    ...
    ├─────────────────────────────────────────────┐
    ▼                                             ▼
consolidate_to_identity                  storyline_weekly（调度器，周频 7d，Brief 80）
    │ 更新 user_identity.yaml                │ 读同一批 episodic（含已被 identity 消费的）
    └─ 标记 episodic.consolidated_at，       │ + storyline_inbox.json（episodic 上限裁剪
       重置 fixation_state                  │   淘汰批次，见下）+ 过滤后的 event_log
                                             └─ open_arc/append_node/set_arc_status → storyline.json
```

两条出口互不排斥：同一条 episodic 既可以被 `consolidate_to_identity` 提炼进稳定属性，
也可以被 `storyline_weekly` 提炼成叙事弧节点——`identity` 回答"是什么样的人"，
`storyline` 回答"在经历什么弧线"，写入路由层（各自的 LLM prompt）显式排除对方的内容类型。
episodic 上限裁剪淘汰的条目不再走 `consolidate_to_identity`/`reflect_to_episodic` 链，
而是由 `handler_storyline_evicted_input` 暂存进 `storyline_inbox.json`，作为
`storyline_weekly` 的第二路输入（详见「四点六、storyline 叙事弧层」）。
...
| `fixation_pipeline.handler_storyline_evicted_input()` | `storyline` | `inbox` | 每次上限裁剪淘汰批次暂存进 inbox 后，`trigger_signal="evict_to_inbox"`（Brief 80 §3，取代已删除的 `digest_evicted_episodes`） |
| `storyline.open_arc()` / `append_node()` / `set_arc_status()` | `storyline` | `{arc_id}` | 每次弧线写操作，`trigger_signal` 为对应操作名（Brief 80 §1） |
