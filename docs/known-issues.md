...
### D2：调度器活跃窗口决策前移（R2-B 已完成；R2-C/D 待办）

**状态**：`refactor-phase`（R2-B 完成 2026-06-11；R2-C/D 待办：legacy trigger 迁移、旧 ready/mark 清理、defer 队列、DND 主入口接线）

**位置**：`core/scheduler/loop.py` → `_pipeline_send()` / `core/scheduler/gating.py`

R2-A 已完成审计（2026-06-10）。

R2-B 已完成（2026-06-11）：active-window / DND 决策已前移到 `gating._decide()` + loop helpers；
`policy.py` 现为 active-window 决策单一权威。`_legacy_active_window_blocks()` /
`_legacy_dnd_blocks()` 作为过渡安全网保留，R2-C 迁移完成后可删除。
守卫测试：`tests/test_r2b_active_window_gating.py`。

**R2-C/D 剩余待办**：
- legacy trigger 代码从直接调用迁移到 gating 仲裁；
- 旧 `_is_ready` / `_mark` 清理；
- defer 队列接线；
- DND 主入口（handle_message）完整接线。
...
- scheduler policy 表已接线（R2-B done）；`_pipeline_send` legacy 安全网在 R2-C 迁移后可删除。
