...
### D2：调度器活跃窗口仍硬编码为 120 秒（pending R2-B）

**状态**：`refactor-phase`（R2-A 已审计，等待 R2-B 施工）

**位置**：`core/scheduler/loop.py` → `_pipeline_send()` → `_user_active_recently()`

低优先级主动消息在用户 120 秒内活跃时跳过。活跃窗口拦截位于 `_pipeline_send()`，与
gating 的 state-machine 决策（CHATTING/QUIET/RESTLESS）分裂，形成双重门。

R2-A 已完成审计（2026-06-10）：

- 双执行面路径图、触发器分类表、决策位置表、policy.py 命运建议均已写入 `docs/scheduler.md` §R2-A 节
- 测试守卫：`tests/test_r2a_scheduler_execution_surface_audit.py`
- 结论：`core/scheduler/policy.py` 是预先设计的 R2-B 接线 schema（非死代码），R2-B 应把
  active-window + `_HIGH_PRIORITY_TRIGGERS` 并入 `POLICY_TABLE`，`_pipeline_send` 退化为纯执行器

**已知 mismatch（R2-B 修复）**：
- `birthday_eve/afternoon/night`：policy.py 说 `defer`，实际行为是 `exempt`（在 `_HIGH_PRIORITY_TRIGGERS`）
- 120s 窗口长度未暴露到配置

**R2-B 建议**：见 `docs/scheduler.md` §R2-A → R2-B 最小施工边界。
