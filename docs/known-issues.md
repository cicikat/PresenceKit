...
### PB4：Path B 降级观察期（Brief 35，一个月，到期 2026-08-10）

**状态**：`observe`

**位置**：`core/pipeline.py::_parse_and_execute_intent`；开关 `config.intent_reflex.enabled`（默认 `false`）。

**背景**：此前审计认为 Path B（角色回复文本反向解析
出桌面操作再执行）与 tool loop（Path C，Brief 28）功能重叠，应整体删除；裁定按两步走节奏落地：
第一步（本条目）只加 config 默认关，五道守卫、c2 幂等窗口、相关测试**本步全部保留**，不删代码。

**观察期**：2026-07-10 至 2026-08-10（一个月）。期间若 tool loop + 探针覆盖不足，出现
"角色说了要做但没做"且用户可感的缺口，**在此登记具体场景**（触发消息、期望动作、实际结果）：

（观察期内如有发现，追加在这行下面）

**到期后**：若无缺口记录，出第二步 brief，整删 `_parse_and_execute_intent` + 五道守卫 +
c2 幂等窗口 + `tests/test_intent_grounding.py` 等相关测试，以及本条目自身（记录完成使命即可删）。
若有缺口，先判断是否该缺口应由 tool loop 侧补齐，而非保留 Path B。
...
入队 `trait_tracker_update` slow_queue 任务，handler 独立于后来已删除的 `character_growth.update()`
...
R8-E2 已落地（2026-06-10）：`character_growth.update()` 已删除，其内部 `trait_state()` 死代码写路径（缺 char_id）随之消除。当时遗留模块仅为只读 legacy 接口，写入链完全由 `trait_tracker_update` slow_queue task 承接；该模块随后于 Brief 35 整体删除。

R8-E3 已落地（2026-06-10）：当时的 `character_growth.py` 模块 docstring 及 `load()` docstring 已明确标注 read-only legacy compatibility surface，并注明禁止重引入 `update()` / `should_update()`。随后模块、`get_growth` 工具和对应守卫已于 Brief 35 一并删除；当前说明见 `docs/memory.md`。
...
`read_diary`、`read_watch`、`search_diary`、`get_profile`、`get_episodic`
...
**状态**：`fixed`（Brief 47，2026-07-11）

**位置**：`core/memory/fixation_pipeline.py` → `_decay_counter_evidence()` / `_synthesize_identity()`

`counter_evidence_count` 曾经只在 LLM 重写 text 时归零，否则只增不减，模型长期保守时
某个维度可能被历史反证永久压死。现在 `consolidate_to_identity()` 在每次合成前先按
`last_conflict_at` 做半衰期（30 天）衰减（`decayed = counter * 0.5 ** (days/30)`，floor
到整数，<1 归零并清 `last_conflict_at`；旧数据缺 `last_conflict_at` 不衰减）再参与判断，
衰减结果写回磁盘并落 `provenance_log`（`trigger_signal="counter_decay"`，仅值变化时记）。
...
| `character_growth.load()` / `character_growth.py` 模块 / `get_growth` 工具 | REMOVED Brief 35 | 唯一读者确认零其他调用方后整体删除（详见 §四点五 / `docs/memory.md`） |
| `consolidate_to_growth` | REMOVED R8-E1 | 已从 `LEGACY_TASK_TYPES` 移除；从未注册 handler，无 enqueue，无 DLQ 文件 |
| `mid_term_append` | REMOVED Brief 35 | handler + 注册已删除（DLQ 确认无残留，见 TD-3） |
| `episodic_compress` | REMOVED Brief 35 | handler + 注册已删除（DLQ 确认无残留，见 TD-3） |
| `LEGACY_TASK_TYPES` | REMOVED Brief 35 | frozenset 本体已删除，随附的 `time_based` DLQ 30 天扫描分支一并删除（见 TD-3） |
...
- `character_growth.py` 曾是只读 legacy 接口（不含 `char_id="yexuan"` 默认参数），随后已于 Brief 35 删除

**保留至 30-day TTL 后再评估**：
- `mid_term_append` / `episodic_compress` handler 注册（见 TD-3）

### TD-3：DLQ legacy handler 兼容层（R8-A）——已随 Brief 35 退役

**状态**：`fixed`（Brief 35，2026-07-10）

**位置**：`core/post_process/slow_queue.py`、`core/scheduler/triggers/time_based.py`

R8-D 实测确认当前 DLQ 目录无任何 legacy task 文件（`mid_term_append` / `episodic_compress`），
30-day TTL 归零已提前达成（无 legacy 积压）。Brief 35 据此按引用计数执行了 R8-E 式最终退役：

- `_handler_mid_term_append` / `_handler_episodic_compress`（`core/pipeline.py`）及其注册已删除。
- `LEGACY_TASK_TYPES` frozenset 与 `is_dlq_task_expired()`（`core/post_process/slow_queue.py`）已删除。
- `_check_dlq_monitor()` 里对应的 30 天过期扫描分支（`core/scheduler/triggers/time_based.py`）已删除，
  DLQ 监控回到单纯的"文件数 > 0 就 log warning"，不再区分 legacy/live 类型。
- 相关测试（`tests/test_r8a_dlq_expiry.py`、`tests/test_r8d_legacy_growth_audit.py` 全文件，
  以及 `test_post_process_ordering.py` / `test_r8b_trait_tracker_queue.py` 里的注册断言）随功能一并删除。

R8-E1（2026-06-10）：`consolidate_to_growth` 已从 `LEGACY_TASK_TYPES` 移除 —
它从未注册 handler，从未 enqueue，DLQ 目录无任何存量文件，属 DEAD 名字残留（历史记录，`LEGACY_TASK_TYPES` 本身现已整体删除）。
...
2. 早期审计曾发现情景召回第一跳偏弱，且 recall_trace 实测同 3 条高强度情绪记忆重复入选、`kw` 为空。后续召回改造已落地；若本问题复现，应以当前 trace 与 `docs/memory.md` 为准重新诊断，而不是采用旧审计结论。主动 diary 触发时，prompt 里同时堆着【用户日记】【叶瑄记录】+ 一堆与当前无关的情绪情景记忆，三者标签隔离弱 → 模型把召回的记忆/自己的推断**并进"你的日记"**复述。
