...
```json
{
  ...
  "move_history": [
    ...
    {"x": 8, "y": 8, "player": "white", "move_no": 2, "source": "ai", "style": "balanced"}
  ],
  "status": "active",
  "winner": null,
...
| `move_history` | list | 按落子顺序；AI 落子额外含 `source="ai"` / `style` |
| `status` | string | `"active"` / `"completed"` |
| `winner` | string\|null | `"black"` / `"white"` / `null` |
| `last_move` | dict\|null | 最近一步落子信息 |
| `opponent` | string | `"human"` / `"yexuan_ai"` |
| `ai_player` | string | AI 执子颜色，P1 固定 `"white"` |
| `ai_style` | string | `"balanced"` / `"gentle"` / `"serious"` / `"teaching"` |
...
`opponent` 和 `ai_style` 为可选字段，不传时默认 `"human"` / `"balanced"`（P0 行为）。
...
**错误：** 422（board_size != 15 / opponent 不合法 / ai_style 不合法）
...
落子。`opponent=yexuan_ai` 时，用户落子成功后后端自动追加 AI 落白一手（除非用户已赢）。
...
- `pending_ai_turn` 字段：对话后标记下一手 AI 落子受 `ai_style_tilt` 影响
- 对话影响下一手 AI 风格（临时覆盖 session 的 `ai_style`）
...
| `core/activity/gomoku_companion.py` | 活动内对话 LLM 生成（generate_reply + prompt 构造 + control 解析） |
...
| `tests/test_gomoku_companion.py` | P0 companion chat 测试（17 用例） |
