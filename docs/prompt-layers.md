...
Reality `1_system_prompt` 绑定当前角色，并使用用户所选称谓（默认“她”）注入非助手、无需服务式官腔或自证价值的定位；不再注入框架人称说明。角色卡、资料与工具返回不经过全文代词替换。builder 的 mood 提示显式使用“你此刻”，管理面命名显示兼容保留。来源地图见 [prompt-unification-audit.md](prompt-unification-audit.md)。
...
| `1_system_prompt` | 角色存在性定义 + 情绪软提示 + `{perception_block}` 槽位 + 不确定时自己查屏幕/记忆/工具结果/{user_pronoun}资料的提醒 | always | 当前角色卡（由 `AssetRegistry` / character loader 解析）+ `core/mood_text.py` |
| `1.5_fact_boundary` | 简短来源边界：按自述/资料/工具各自时间理解；无屏幕快照不否定其他来源，桌宠形象归属于角色；电量/步数/餐食/行为须确认事件，不把过时信息当成当前 | always | 不复制桌面摘要，不宣称其他现实事实未知 |
...
4. **输出端校验重试（硬止血）**（`core/pipeline.py::Pipeline._anti_collapse_prefix_retry()`，`run_llm()`
   ...
   P 后接受，非填充词前缀只接受重试结果不做硬剥离。仅覆盖 `run_llm()`（非流式）路径。开关
   `anti_collapse.prefix_retry`（默认 `true`）；重试计入一次额外 LLM 调用成本，日志 `[anti_collapse] prefix retry`。
   `run_llm_stream()` 流式路径不能套用（已实时吐出的 token 无法撤回丢弃重试），走独立的软降级方案，
   见下方「ACT-2」。

### ACT-2 流式路径反坍缩：软降级（方案 B，`cc-tasks/105`）

流式 token 已对用户可见，"用户看到又收回"比"偶尔一次重复开头"体验伤害更大；坍缩是趋势性问题，
跨轮纠正足够，不必单轮内救——因此流式路径不做"暂缓前 N token 再决定丢弃"（方案 A，已封存，
除非上线后单轮坍缩率显著高于非流式再复议），只做跨轮软降级：

1. **检测**（`core/pipeline.py::Pipeline.run_llm_stream()` 正常收完整个流后）：复用与 S2 同一份
   `detect_reply_homogeneity_prefix()`（`Pipeline._homogeneity_hist_for_check()` 组装 hist，与
   `_anti_collapse_prefix_retry()` 共用同一 helper），命中时**不中断、不丢弃本轮已发内容**。
2. **记录**（`Pipeline._check_stream_collapse()`）：打观测日志
   `[anti_collapse] stream_soft_degrade uid=%s char_id=%s rule=homogeneity_prefix prefix=%r`；
   调用 `core/memory/short_term.py::note_stream_collapse_signal(user_id, prefix, char_id=)`
   写入一次性信号 `data/runtime/memory/{char_id}/{uid}/stream_collapse_signal.json`（路径经
   `core/sandbox.get_paths().stream_collapse_signal()`，未硬编码）。
3. **下一轮消费**：`build()` 组装层 `stream_collapse_hint`（`_drop_priority=95`）时调用
   `consume_stream_collapse_signal(user_id, char_id=)`——读到即删，一次性消费，不做像 S3/S4 那样的
   `hint_rounds` 持久化倒计时。填充词前缀复用 S2 同款"不复读字面"文案，非填充词前缀复用引用式文案。
4. 无需 `user_id` 时（`run_llm_stream()` 调用方未传）整段检测跳过——没有 uid 就无法定位下一轮读取
   路径，fail-open 不影响本轮输出。
...
| `"stage"` | 群聊标记 | Stage 群聊生成；额外字段 `group_id`、`speaker`、`round_id`，供管理面板按群过滤 |
| `"private_exchange"` | 私下往来标记 | 角色私下往来生成；额外字段 `pair`、`speaker`，不带 `group_id` |
...
Reality prompt 环形缓冲按 uid 保存最近 10 轮，足够群聊页聚合最近 10 次生成；进程重启后清空。
