...
def test_memory_event_query_page_is_registered_and_cache_busted():
    ...
    assert "ADMIN_UI_FRAGMENT_VERSION = 'admin-prompt-followups-1'" in core
    assert '<script src="/static/js/core.js?v=admin-prompt-followups-1"></script>' in index
    assert '<script src="/static/js/observability.js?v=brief-242-settings-4"></script>' in index
    assert '<script src="/static/i18n.js?v=admin-prompt-followups-1"></script>' in index
...
def test_shadow_recall_rollout_controls_are_registered_and_cache_busted():
    ...
    assert "ADMIN_UI_FRAGMENT_VERSION = 'admin-prompt-followups-1'" in core
    assert '<script src="/static/js/settings.js?v=vision-protocol-2"></script>' in index
