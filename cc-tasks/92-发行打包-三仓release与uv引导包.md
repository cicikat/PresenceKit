...
- (20260718 真机实测补充)README「快速开始」需含**首次安装完整步骤清单**,
  按 Brief 93 落地后的新流程写:下载两个 release 包 → 解压到**非系统盘**
  (建议 D 盘)→ AA1(装环境+启动)→ AA2(鉴权初始化,自动弹密钥本+管理面板)→
  面板「配置」页填必填项(基础聊天 API,标红)→ desktop 首启引导页粘贴 token。
  不再指导用户手改 config.yaml。
- (20260718 真机实测补充)下载说明:Edge 会拦截未签名包且「保留」不一定生效,
  建议 Chrome 下载或按微软官方路径手动保留;SmartScreen「更多信息→仍要运行」。
  与 desktop 仓 Brief 34 §4 文案保持一致、互链。
