...
**Activity naming**: `core/activity_manager.py` owns ambient presence (the character's current activity, prompt layer `2.6_presence`); `core/activity/` owns user-started structured `ActivitySession` co-play. They are unrelated systems; see `docs/activity-session.md`.
