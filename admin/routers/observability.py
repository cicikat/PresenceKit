...
class _BrowserTaskRequest(BaseModel):
    uid: str
    char_id: str
    url: str
    operation: str
...
@router.post("/agent-runtime-browser/tasks", summary="Create a bounded Reality browser task")
async def create_agent_runtime_browser_task(body: _BrowserTaskRequest, _auth=Depends(require_scopes("chat"))):
...
@router.post("/agent-runtime-browser/tasks/{task_id}/run", summary="Run a confirmed browser task")
async def run_agent_runtime_browser_task(task_id: str, body: _BrowserTaskRequest, _auth=Depends(require_scopes("chat"))):
    from core.agent_runtime.browser import BrowserError, run_task
    from core.agent_runtime.models import TaskPrincipal
    try:
        return await run_task(TaskPrincipal.reality(body.uid, body.char_id), task_id, url=body.url, operation=body.operation, confirmed=body.confirmed, params=body.params)
    except BrowserError as exc:
        raise HTTPException(status_code=422, detail={"code": exc.code}) from exc


@router.get("/agent-runtime-browser/tasks/{task_id}", summary="Read one browser task receipt")
async def get_agent_runtime_browser_task(task_id: str, uid: str = Query(...), char_id: str = Query(...), _auth=Depends(require_scopes("state.read"))):
    from core.agent_runtime.models import TaskPrincipal
    from core.agent_runtime.task_manager import get_task
    return get_task(TaskPrincipal.reality(uid, char_id), task_id)
