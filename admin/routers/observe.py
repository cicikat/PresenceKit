...
async def get_runtime_state(auth=Depends(require_scopes("memory.read"))):
    return {
...
async def list_prompt_layer_uids(auth=Depends(require_scopes("memory.read"))):
    from core.observe.prompt_capture import list_uids
...
async def get_prompt_layers(uid: str, n: int = 0, auth=Depends(require_scopes("memory.read"))):
...
async def list_probe_uids(auth=Depends(require_scopes("memory.read"))):
...
async def get_probe_snapshot(uid: str, n: int = 0, auth=Depends(require_scopes("memory.read"))):
...
async def list_dream_prompt_uids(auth=Depends(require_scopes("memory.read"))):
...
async def get_dream_prompt_snapshot(uid: str, n: int = 0, auth=Depends(require_scopes("memory.read"))):
...
async def get_trigger_catalog(auth=Depends(require_scopes("memory.read"))):
    try:
...
async def get_vector_overview(uid: str, source: str = "", limit: int = 100,
                              char_id: str = "", auth=Depends(require_scopes("memory.read"))):
