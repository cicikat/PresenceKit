...
async function loadFeatureCenter() {
  ...
  const names={tool_loop:t('settings_center.multi_step_tool_calls',"多步工具调用"),mcp:t('settings_center.external_tools',"外部工具"),self_capability:t('settings_center.self_management',"自主管理"),autonomy:t('settings_center.autonomous_activity',"自主活动"),scheduler:t('settings_center.scheduler',"调度器"),channels:t('settings_center.message_channels',"消息通道"),model_routing:t('settings_center.model_profile',"模型方案"),embedding:t('settings_center.embedding_search',"向量检索"),tts:t('settings_center.speech_synthesis',"语音合成"),hardware_intiface:t('settings_center.hardware_connection',"硬件连接")};
  html+=section(t('settings_center.effective_state',"当前生效状态"),state.status==='fulfilled'?(state.value.features||[]).map(row=>`<p><strong>${escapeHtml(names[row.id]||row.id)}</strong> · ${escapeHtml(t('overview.status.'+row.runtime_status, row.effective_value===true?t('settings_center.currently_available',"当前可用"):row.effective_value===false?t('settings_center.currently_unavailable',"当前不可用"):t('settings_center.status_loaded',"状态已读取")))} ${row.blocking_reason?`<span>· ${escapeHtml(t('overview.reason.'+row.blocking_reason,t('settings_center.blocked_see_details',"存在阻断，查看详情")))}</span>`:''} ${centerLink(row.edit_page||'status',t('settings_center.details',"详情"))}</p>`).join(''):t('settings_center.status_unavailable_this_does_not_mean_the_feature_is_disabled',"状态读取失败；不代表功能关闭"));
