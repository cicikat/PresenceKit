...
10. 有明确时间弧线的过程性叙事（职业方向转变、一个项目的推进、一段持续的情绪历程）不属于这里，
   那是 storyline 层的职责——你只归纳跨情景反复出现、没有特定时间跨度的稳定属性（Brief 80）。
...
def _get_scope_from_payload(payload: dict, handler_name: str) -> MemoryScope:
    ...
    logger.warning(
        "[fixation.%s] payload 缺少 scope/char_id，使用 legacy DLQ fallback char_id=%s "
        "(uid=%s)",
